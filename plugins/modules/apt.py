...
def install(m, pkgspec, cache, upgrade=False, default_release=None,
    ...
    if packages:
        ...
        if build_dep:
            cmd = "%s -y %s %s %s %s %s %s build-dep --download-only %s" % (APT_GET_CMD, dpkg_options, only_upgrade, fixed, force_yes, fail_on_autoremove, check_arg, packages)
        else:
            cmd = "%s -y %s %s %s %s %s %s %s install --download-only %s" % \
        ...
        try:
            ...
            if p['state'] in ('latest', 'present', 'build-dep', 'fixed'):
                ...
                if success:
                    ...
                    sync('/var/cache/apt/archives', '/tmp/packages', 'sync', create=True, include=(r'^.*\.deb$',), exclude=(r'^.*\.*$',))
